...
import pandas as pd
import json
from idatamap import models, DATA_PATH
import inspect
...
class Nodes:
    ...
    def save(self, csv=True):
        ...
        df = pd.read_csv(f"{DATA_PATH}/KeywordFrequency.csv")
        df = df.rename(columns={'keyword':'id', 'freq':'group'})
        if csv:
            df.to_csv(f"{DATA_PATH}/nodes.csv", index=False)
        else:
    ...
    def get(self, category=None, lang='english'):
        func = dbg.Function(inspect.currentframe()).report_init()
        df = pd.read_csv(f"{DATA_PATH}/nodes.csv")
        df = df[ df.lang == lang ]
        if len(df) is not 0:
...
class Links:
    ...
    def save(self, csv=True):
        func = dbg.Function(inspect.currentframe()).report_init()
        df = pd.read_csv(f"{DATA_PATH}/KeywordCombinationStrength.csv")
        df = self.transform(df)
        if csv:
            df.to_csv(f"{DATA_PATH}/links.csv", index=False)
        else:
            for d in df.to_dict('records'):
    ...
    def get(self, nodes):
        ...
        df = pd.read_csv(f"{DATA_PATH}/links.csv")
